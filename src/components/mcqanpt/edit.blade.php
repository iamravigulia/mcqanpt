...
@php $que = DB::table('fmt_mcqanpt_ques')->where('id', $message)->first(); @endphp
<div class="fixed z-10 inset-0 overflow-y-auto hidden" id="modalMCQANPT{{$que->id}}">
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      -->
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full relative -mx-8" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <a onclick="closeModalMCQANPT({{$message}})" style="z-index: 999999999999999999999999; position: relative;" class="p-2 bg-white w-8 h-8 bg-gray-600 text-white rounded-full absolute right-0 -top-10 -mr-2 -mt-2 z-40" href="javascript:void(0);">x</a>
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <form action="{{route('fmt.mcqanpt.update', $que->id)}}" method="post" enctype="multipart/form-data">
                    @if ($errors ?? '')
                        <div class="my-4">
                    ...
                    @endif
                    @csrf
                    <div class="text-xl">Edit MCQAnPT Question {{$message}}</div>
                    ...
                    @php $answers = DB::table('fmt_mcqanpt_ans')->where('question_id', $que->id)->pluck('id'); @endphp
                    @foreach ($answers as $ans)
                        @php $answer = DB::table('fmt_mcqanpt_ans')->where('id', $ans)->first(); @endphp
                        {{--  --}}
                        <div class="flex flex-wrap my-4 text-xs border-2 border-gray-400" id="ans1">
                            <div class="w-1/2">
                            ...
                            <div class="w-1/2 px-4">
                                <label class="w-full" for="">Replace Media</label>
                                <input class="block my-3 w-10 h-10" type="file" name="answer_media_{{$answer->id}}" id="" @if ($answer->arrange == 1) checked @endif>
                            </div>
                            <div class="w-1/2 px-4">
                                <label class="w-full" for="">Correct</label>
                                <div class="flex bg-white">
                                    @php $ans_media = DB::table('media')->where('id', $answer->media_id)->first() @endphp
                                    <img src="{{url('/')}}/storage/{{$ans_media->url  ?? "none"}}" style="padding:0 4px; width:40px; margin:14px 10px 0 0; height:30px; object-fit:cover;"></li>
